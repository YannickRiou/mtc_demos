...
void spawnObject(moveit::planning_interface::PlanningSceneInterface& psi) {
	...
	o.primitive_poses[0].position.x = 1.2;
	o.primitive_poses[0].position.y = 0.4;
	o.primitive_poses[0].position.z = 0.75+(0.20/2)+0.1;
...
void planTest(Task &t) {
	...
	{
		// connect to pick
		stages::Connect::GroupPlannerVector planners = {{"left_gripper", pipeline}, {"left_arm", pipeline},{"torso", pipeline},{"base", pipeline}};
...

	{
		...
		pick->setProperty("group","left_arm_base");
	...
	/*{
		// connect to pick
		stages::Connect::GroupPlannerVector planners = {{"left_gripper", pipeline}, {"left_arm", pipeline},{"torso", pipeline}};
		auto connect = std::make_unique<stages::Connect>("connect", planners);
		connect->properties().configureInitFrom(Stage::PARENT);
		t.add(std::move(connect));
	}

	{
		...
		p.pose.position.z=  0.75+(0.20/2)+0.1;
		stage->setPose(p);
		stage->setObject("object");
		stage->setProperty("eef", "left_gripper");
		stage->setProperty("group","left_arm_torso");
		...
		wrapper->setProperty("group","left_arm_torso");
