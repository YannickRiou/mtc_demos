...
void createPickTask(const geometry_msgs::PoseStamped objPose,bool customGraspPose, const geometry_msgs::PoseStamped graspPose,Task &pickTask) {

	pickTask.loadRobotModel();
...

	{
		...
		pickTask.add(std::move(wrapper));
	}

  std::cerr << pickTask << std::endl;
	pickTask.plan(10);
}
